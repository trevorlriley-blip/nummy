...
import { healthIssueOptions } from '../../src/data/onboardingOptions';
import { HealthIssue } from '../../src/types/user';
...
export default function HealthIssuesScreen() {
  ...
  const [selected, setSelected] = useState<HealthIssue[]>(
    (state.partialPreferences.healthIssues as HealthIssue[]) ?? []
  );

  const handleToggle = (value: string) => {
    const issue = value as HealthIssue;
    if (issue === 'none') {
      setSelected(['none']);
      return;
    }
    setSelected((prev) => {
      ...
      return without.includes(issue) ? without.filter((v) => v !== issue) : [...without, issue];
    });
  };

  const handleNext = () => {
    updatePartialPreferences({ healthIssues: selected });
    goToNextStep();
    router.push('/onboarding/dietary-goals');
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Text variant="bodyLarge" style={[styles.subtitle, { color: theme.colors.onSurfaceVariant }]}>
          We'll prioritize recipes that support your health needs.
        </Text>

        <ChipSelector
          options={healthIssueOptions.map((o) => ({ value: o.value, label: o.label, icon: o.icon }))}
          selected={selected}
